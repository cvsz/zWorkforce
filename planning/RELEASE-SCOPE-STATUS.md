...
| State | Meaning |
| --- | --- |
| `v3.0.3 required / complete` | Repository work explicitly required for the current candidate and implemented/verified to the evidence recorded by the release authorities. |
| `v3.0.3 required / incomplete` | A current candidate blocker that must be fixed before immutable promotion. |
...
| --- | --- | --- |
| zWorkforce control plane — `exec-planning-zwf.md` | `v3.0.3 required / complete` for repository candidate preparation; `external evidence` remains | Canonical current-release repository scope. Final immutable promotion still requires exact-candidate checks/reviews and all mandatory external evidence. |
