(function attachZarvisVoiceClient(root) {
  ...
  function bindPushToTalk({ button, keyTarget = root, onStart, onStop, onCancel, editable = isEditableTarget }) {
    ...
    const keyDown = (event) => {
      if (event.code === "Escape") { onCancel?.(event); return; }
      if (event.code === "Space" && !event.repeat && !editable(event.target)) { event.preventDefault?.(); start(event); }
    };
    const keyUp = (event) => {
      if (event.code === "Space" && !editable(event.target)) { event.preventDefault?.(); stop(event); }
    };
