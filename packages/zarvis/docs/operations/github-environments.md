...
| `SUPABASE_TABLE` | Exposed Supabase table queried by the ZARVIS API |
...
The ZARVIS API reads these Supabase values from the loaded `.env`, `.env.phase6`, and `.env.phase6.server` overlays when the helper script refreshes the GitHub environment. Only the anon key is sensitive; the URL and table name are operational configuration.
