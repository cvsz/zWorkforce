...
class WorkspaceEvidenceApiTests(unittest.TestCase):
    ...
    def test_sidecar_projects_durable_evidence_without_raw_sensitive_payloads(self):
        ...
        self.assertEqual(payload["returned_counts"]["artifacts"], 1)
        self.assertEqual(payload["returned_counts"]["subtasks"], 1)
        self.assertEqual(payload["returned_counts"]["workflow_refs"], 1)
        self.assertEqual(payload["limits"]["artifacts"], 500)
        self.assertFalse(payload["possibly_truncated"]["artifacts"])
